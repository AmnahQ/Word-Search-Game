...
class WordSearch:
    def __init__(self, root):
        ...
        for row in self.board:
            print(" ".join(row))  # debugging 
        print("\nSolving Word Search board...")
        self.valid_words = set(word.lower() for word in grid_solver.solve_board(self.board))
        print("\nValid Words Found:", self.valid_words)  # Debugging 
    ...
    def create_screen(self):
        ...
        # self.board_frame.configure(bg="#B0E0E6")
        ...
        self.submit_button = Button(self.button_frame, text="Submit Word",bg="white", command=self.submit_word)
        self.submit_button.pack(side=LEFT, padx=5)
        
        self.clear_button = Button(self.button_frame, text="Clear Word",bg="white", command=self.clear_current_word)
        self.clear_button.pack(side=LEFT, padx=5)

        # Game score timer result display
        ...
        self.end_button = Button(self.root, text="End Game", bg="white", command=self.end_game)
    ...
    def letter_clicked(self, row, col):
        ...
        if (row, col) in self.clicked_buttons:
            return  # so user only clicks one letter tile once in a word
        ...
        self.letter_buttons[row][col].config(bg="lightblue")

        # Tracking the placemment of the clicked letter
        self.clicked_buttons.append((row, col))
    ...
    def clear_current_word(self):
        ...
        # Reset button colors
        for row, col in self.clicked_buttons:
            self.letter_buttons[row][col].config(bg="SystemButtonFace")
        self.clicked_buttons = []
    ...
    def end_game(self):
        ...
        # Restart button
        restart_button = Button(popup, text="Restart Game", font=("Courier", 12), command=self.restart_game)
